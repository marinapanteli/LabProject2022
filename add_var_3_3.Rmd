...
# Check strand direction and convert to + if -
...
st=variation_regs[nm]@ranges@start
...
ga_k <- ga[names(ga) %in% rds] #HERE #subset for transcript of interest

na <- c(na, as.data.frame(narrowAlignments(ga_k, gr)@elementMetadata)) #FIND BETTER WAY TO STORE THE VARIATIONS OF ALL READS ACROSS ALL POSITIONS OF VARIATIONS (0)
#mcols(na)$seq

}

z<-matrix(unlist(na), ncol=length(na))

#CREATE z variation combinations (1)

#CREATE alleles (2) with below
